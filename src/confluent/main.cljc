(ns confluent.main
  "Kotodama WASM entrypoint for the Confluent clean-room actor (L4) — Clojure port.
  ...
  facts (`confluent.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "confluent")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"confluent.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "confluent-compat" :tier tier :entities entities} 200])
...
(defn start! [] :confluent-compat/ready)
